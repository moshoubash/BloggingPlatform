<x-app-layout>
    ...
    <div class="relative flex items-top justify-center py-4 sm:pt-0">
        <div class="max-w-5xl w-full mx-auto sm:px-6 lg:px-8 my-8 md:my-16">
            <article itemscope itemtype="http://schema.org/Article" class="bg-white rounded-lg shadow-md dark:bg-gray-800 py-4 px-6 dark:text-white">
                ...
                @if ($post->is_premium && (!auth()->check() || !auth()->user()->hasPremiumSubscription()))
                    <div class="relative mb-10">
                        ...
                        <div class="absolute top-0 left-0 right-0 z-10 text-center py-10 bg-gradient-to-br from-gray-100/90 to-gray-300/90 dark:from-gray-900/90 dark:to-gray-700/90 rounded-lg shadow-xl backdrop-blur-sm">
                            <h2 class="text-4xl font-extrabold text-gray-800 dark:text-gray-100 mb-4">🔒 Premium Content</h2>
                            <p class="text-lg text-gray-600 dark:text-gray-300 mb-6">
                                Subscribe now to unlock full access to this premium article.
                            </p>
                            <form action="{{ route('checkout.process') }}" method="POST" class="mt-8">
                                @csrf
                                <button class="px-6 py-3 text-sm font-medium text-white bg-gradient-to-r from-indigo-700 to-purple-700 rounded-lg hover:from-indigo-800 hover:to-purple-800 transition shadow-md hover:shadow-lg">
                                    Subscribe Now
                                </button>
                            </form>
                        </div>

                        <!-- Preview Content - First Part Visible, Rest Blurred -->
                        <section itemprop="articleBody" class="prose dark:prose-invert max-w-none pb-3">
                            <div class="mb-10">
                                {!! Str::limit(strip_tags($markdown), 200) !!}...
                            </div>
                    ...
                    <section id="likesAndBookmarks" class="border-t-2 dark:border-gray-600 mt-5 pt-5 pb-2 flex flex-row justify-between items-center" style="gap: 1rem;">
                        <div class="flex items-center">
                            <form action="{{ route('posts.like', ['post' => $post]) }}" method="POST">
                                @csrf
                                <button type="submit" class="flex items-center space-x-2 bg-blue-500 hover:bg-blue-600 transition text-white rounded-lg px-4 py-2" title="Like">
                        ...
                        <div class="flex items-center">
                            <form action="{{ route('posts.bookmark', ['post' => $post]) }}" method="POST">
                                @csrf
                                <button type="submit" class="flex items-center space-x-2 bg-gray-200 dark:bg-blue-500 hover:bg-gray-300 dark:hover:bg-blue-600 transition text-gray-800 dark:text-white rounded-lg px-4 py-2" title="Bookmark">
                    ...
                    <footer>
                        ...
                        @if (config('blog.allowComments'))
                            <section id="comments" class="border-t-2 dark:border-gray-600 mt-5 pt-5 pb-2">
                                    ...
                                    <ul>
                                        @foreach ($post->comments as $comment)
                                            <li id="comment-{{ $comment->id }}" class="rounded-lg bg-gray-200 dark:bg-gray-700 p-4 my-4 relative group">
                                                <div>
                                                    <a href="{{ route('posts.index', ['author' => $comment->user]) }}" class="font-medium">
                                                        <small class="opacity-75">@</small>{{ $comment->user->name }}:
                                                    </a>
                                                    @if ($comment->published_at != $comment->updated_at)
                                                        <i class="text-xs opacity-75 ml-2" title="This comment was last updated {{ $comment->updated_at }}">Edited</i>
                                                    @endif
                                                </div>
                                                ...
                                                @auth
                                                    @if (Auth::id() === $comment->user_id || Auth::user()->is_admin)
                                                        <div class="absolute top-3 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                            <a class="font-semibold dark:font-medium mr-2 text-blue-600 dark:text-blue-400 hover:underline" href="{{ route('comments.edit', ['comment' => $comment]) }}">Edit</a>
                                                            <form action="{{ route('comments.destroy', ['comment' => $comment]) }}" method="POST" class="inline" onSubmit="return confirm('Are you sure you want to delete this comment?')">
                                                                @method('DELETE')
                                                                @csrf
                                                                <button type="submit" class="font-semibold dark:font-medium text-red-600 dark:text-red-500 hover:underline">Delete</button>
                                                            </form>
                                                        </div>
                                                    @endif
                                                @endauth
                                ...
                                @else
                                    <p class="text-gray-600 dark:text-gray-400 italic">No comments yet. Be the first to share your thoughts!</p>
                                @endif
                                    ...
                                    @elseif(Gate::inspect('create', App\Models\Comment::class)->message() == 'Your email must be verified to comment.')
                                        <div class="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                                            <p class="text-yellow-800 dark:text-yellow-200">
                                                ...
                                                <x-link :href="route('verification.notice')" class="font-medium">Resend verification email</x-link>
                                            </p>
                                        </div>
                                    @endcan
                                        ...
                                        <div class="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                                            <p class="text-blue-800 dark:text-blue-200">
                                                ...
                                                @if (Route::has('register'))
                                                    or <x-link :href="route('register')" class="font-medium">sign up</x-link>
                                                @endif
            ...
            @if (config('blog.withTags') && $post->tags && isset($relatedPosts) && count($relatedPosts) > 0)
                <section id="related-posts" class="mt-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($relatedPosts as $relatedPost)
                            <article class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden transition hover:shadow-lg">
                                @if ($relatedPost->featured_image && basename($relatedPost->featured_image) != 'default.jpg')
                                    <div class="h-36 overflow-hidden">
                                        <img src="{{ $relatedPost->featured_image }}" alt="Featured Image for {{ $relatedPost->title }}" class="w-full h-full object-cover">
                                    </div>
                                @endif
                                <div class="p-4">
                                    <a href="{{ route('posts.show', $relatedPost) }}" class="block hover:text-blue-600 dark:hover:text-blue-400 transition">
                                    ...
                                    <div class="mt-4 text-sm text-gray-500 dark:text-gray-400 flex items-center">
                                        <a href="{{ route('profile', ['user' => $relatedPost->author]) }}" class="hover:text-blue-600 dark:hover:text-blue-400 transition">{{ $relatedPost->author->name }}</a>
